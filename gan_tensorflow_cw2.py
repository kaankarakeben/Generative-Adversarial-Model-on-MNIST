...
def discriminator(x, drop_out):
    ...
    h0 = tf.nn.leaky_relu(tf.matmul(x, w0) + b0)
    h0_do= tf.nn.dropout(h0, drop_out)  # DROP-OUT
    # 2nd hidden layer
    w1 = tf.get_variable('D_w1', [h0_do.get_shape()[1], 512], initializer=w_init)
    b1 = tf.get_variable('D_b1', [512], initializer=b_init)
    h1 = tf.nn.leaky_relu(tf.matmul(h0_do, w1) + b1)
    h1_do= tf.nn.dropout(h1, drop_out)  # DROP-OUT
    # 3rd hidden layer
    w2 = tf.get_variable('D_w2', [h1_do.get_shape()[1], 256], initializer=w_init)
    b2 = tf.get_variable('D_b2', [256], initializer=b_init)
    h2 = tf.nn.leaky_relu(tf.matmul(h1_do, w2) + b2)
    h2_do= tf.nn.dropout(h2, drop_out)  # DROP-OUT
    # output layer
    w3 = tf.get_variable('D_w3', [h2_do.get_shape()[1], 1], initializer=w_init)
    b3 = tf.get_variable('D_b3', [1], initializer=b_init)
    o = tf.sigmoid(tf.matmul(h2_do, w3) + b3)
    ###  Code: ToDO( Change the architecture as CW2 Guidance required)
